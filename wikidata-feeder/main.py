...
def main():
    ...
    if args.loop:
        logger.info("Running in loop mode until all pending artworks are enriched...")
        while True:
            pending_count = status_collection.count_documents({"status": "PENDING_WIKIPEDIA"})
            if pending_count == 0:
                logger.info("All artworks enriched. Exiting loop.")
                break
